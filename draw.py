...
def def_parser():
    ...
    parser.add_argument('-i', '--input', dest='i', help='Demands file (default is demands.txt)',
                        type=str, default='demands.txt')
    ...
    parser.add_argument('-n', '--no', dest='n', help='No id in demand file',
                        action='store_true')
    return parser
...
def write_to_file(handle, workload):
    for i in range(0, len(workload)):
        handle.write("%d%s" % (workload[i], NEWLINE))
